...
import 'package:dragonfly_builder/builder/code_builder/builder_utils.dart';
...
import 'package:dragonfly_builder/builder/models/external_module_config.dart';
...
import 'package:dragonfly_builder/builder/models/module_config.dart';
import 'package:dragonfly_builder/builder/utils/utils.dart';

const _injectableImport = 'package:dragonfly/dragonfly.dart';
const _getItImport = _injectableImport; // 'package:get_it/get_it.dart';
const _getItRefer = Reference('DragonflyContainer', _injectableImport);
const _ghRefer = Reference('DragonflyContainerHelper', _injectableImport);
          ...
          [
            ...
            if (!isMicroPackage) ...[
              if (asExtension)
                Extension(
                  (b) => b
                    ..name = 'DragonflyContainerInjectableX'
                    ..on = _getItRefer
...
class InitMethodGenerator with SharedGeneratorCode {
  ...
  Method generate() {
    ...
    final ghBuilder =
        refer('DragonflyContainerHelper', _injectableImport).newInstance(
      [
        getInstanceRefer,
        refer('environment'),
          ...
          (b) => b.statements.addAll([
            if (scopeName != null)
              ...
              if (usesConstructorCallback)
                declareFinal('ccb')
                    .assign(
                      refer('constructorCallback').ifNullThen(
                          const CodeExpression(Code('<T>(_) => _'))),
                    )
                    .statement,
              ...ghStatements,
